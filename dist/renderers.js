const esc = (v) => v == null ? "" : String(v)
    ...
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#39;");
const tw = (...cs) => cs.filter(Boolean).join(" ");
const alignFlex = (a) => a === "left" ? "justify-start" : a === "center" ? "justify-center" : "justify-end";
const dateFmt = (iso) => {
    if (!iso)
...
const paraBlock = (props, style) => `
<section class="${tw("mb-3", style?.wrapper)}">
  <p class="${tw("text-justify", style?.text)}">${esc(props.text)}</p>
</section>`;
const dividerBlock = (colors, style) => `
<hr class="${tw("my-4", style?.hr)}" style="border-color:${colors.border}"/>`;
const spacerBlock = (props, style) => {
    const m = { xs: "h-2", sm: "h-4", md: "h-8", lg: "h-12", xl: "h-20" };
    return `<div class="${tw(m[props.size || "md"], style?.wrapper)}"></div>`;
};
const pagebreakBlock = () => `<div class="pagebreak"></div>`;
const signatureBlock = (props, style, colors) => {
    const n = Math.max(1, Math.min(5, props.lines ?? 1));
    const lines = Array.from({ length: n }).map(() => `<div class="border-b" style="border-color:${colors.border};height:2rem;"></div>`).join("");
    return `
...
const footerTextBlock = (props, style) => `
<section class="${tw("mt-8 text-center text-sm text-slate-600", style?.text)}">${esc(props.text)}</section>`;
const tableBlock = (props, style, cfg, colors) => {
    const title = props.title ? `<div class="${tw("mb-2 font-semibold text-slate-800", style?.title)}">${esc(props.title)}</div>` : "";
    const compact = cfg.table.compact ? "py-1 px-2 text-sm" : "py-2 px-3";
    const thead = (props.headers || []).map((h) => `<th class="${compact} border-b font-semibold text-left" style="border-color:${colors.border}">${esc(h)}</th>`).join("");
    const body = (props.rows || []).map((row, i) => {
        const bg = cfg.table.striped && i % 2 === 1 ? "bg-gray-50" : "";
        const tds = row.map((c) => `<td class="${compact} border-b" style="border-color:${colors.border}">${esc(c)}</td>`).join("");
        return `<tr class="${bg}">${tds}</tr>`;
    }).join("");
    const notes = props.notes ? `<div class="mt-2 text-xs text-slate-500">${esc(props.notes)}</div>` : "";
    return `
...
export const renderBody = (r) => {
    const fixedHeader = r.configs.header.visible && r.configs.header.repeat === "all"
        ? `
<header class="fixed-header border-b" style="border-color:${r.colors.border}">
  <div class="flex items-center ${"justify-" + r.configs.header.align} h-full px-4">
    ...
    ${r.assets.headerImage
        ...
        : "";
    const firstPageHeader = r.configs.header.visible && r.configs.header.repeat === "first"
        ? `
...
<footer class="fixed-footer border-t px-4" style="border-color:${r.colors.border}">
  ...
  <span class="text-sm text-gray-600">
    ...
    const parts = r.components.map(c => {
        switch (c.type) {
            case "header": return headerBlock(c.props, c.style);
            case "subheader": return subheaderBlock(c.props, c.style);
            case "date": return dateBlock(c.props, c.style, r.configs);
            case "para": return paraBlock(c.props, c.style);
            case "divider": return dividerBlock(r.colors, c.style);
            case "spacer": return spacerBlock(c.props, c.style);
            case "pagebreak": return pagebreakBlock();
            case "signature": return signatureBlock(c.props, c.style, r.colors);
            case "footerText": return footerTextBlock(c.props, c.style);
            case "table": return tableBlock(c.props, c.style, r.configs, r.colors);
            default: return "";
        }
    }).join("");
    const main = `
