...
const tableBlock = (props: any, style: any, cfg: Cfg, colors: Col) => {
  ...
  const thead = (props.headers||[]).map((h: string)=>`<th class="${compact} border-b font-semibold text-left" style="border-color:${colors.border}">${esc(h)}</th>`).join("");
  const body = (props.rows||[]).map((row: any[], i: number)=>{
    const bg = cfg.table.striped && i%2===1 ? "bg-gray-50" : "";
    const tds = row.map((c: any)=>`<td class="${compact} border-b" style="border-color:${colors.border}">${esc(c)}</td>`).join("");
...
<footer class="fixed-footer border-t px-4" style="border-color:${r.colors.border}">
  ...
  const parts = r.components.map(c => {
    switch (c.type) {
      case "header":     return headerBlock((c as any).props, c.style);
      case "subheader":  return subheaderBlock((c as any).props, c.style);
      case "date":       return dateBlock((c as any).props, c.style, r.configs);
      case "para":       return paraBlock((c as any).props, c.style);
      case "divider":    return dividerBlock(r.colors, c.style);
      case "spacer":     return spacerBlock((c as any).props, c.style);
      case "pagebreak":  return pagebreakBlock();
      case "signature":  return signatureBlock((c as any).props, c.style, r.colors);
      case "footerText": return footerTextBlock((c as any).props, c.style);
      case "table":      return tableBlock((c as any).props, c.style, r.configs, r.colors);
      default:           return "";
    }
  }).join("");
